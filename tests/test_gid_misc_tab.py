...
that decision is built from, using the exact tab-listing shape (and the
exact "Misc" tab name) that yetracker.net's real page embeds.
"""

from src.fetcher import _discover_named_tabs, _get_misc_tabs
...
class TestGetMiscTabs:
    def test_identifies_the_misc_tab_gid(self):
        tabs = _discover_named_tabs(YETRACKER_TAB_LISTING_JS)
        misc_tabs = _get_misc_tabs(tabs)
        assert misc_tabs == [("70063278", "misc")]

    def test_main_and_art_tabs_are_not_misc(self):
        tabs = _discover_named_tabs(YETRACKER_TAB_LISTING_JS)
        misc_gids = {gid for gid, _kind in _get_misc_tabs(tabs)}
        assert "34972268" not in misc_gids  # Unreleased (main tab)
        assert "1219860820" not in misc_gids  # Art

    def test_music_videos_tab_is_classified_separately(self):
        tabs = {"1": "Unreleased", "2": "Music Videos"}
        assert _get_misc_tabs(tabs) == [("2", "music_videos")]

    def test_misc_sorts_before_music_videos(self):
        tabs = {"1": "Music Videos", "2": "Misc"}
        assert _get_misc_tabs(tabs) == [("2", "misc"), ("1", "music_videos")]

    def test_no_misc_or_music_video_tabs(self):
        tabs = {"1": "Unreleased", "2": "Art", "3": "Groupbuys"}
        assert _get_misc_tabs(tabs) == []

    def test_this_is_the_exact_decision_the_gid_shortcut_relies_on(self):
        """Mirrors `gid_is_misc_tab = gid in {g for g, _k in _get_misc_tabs(named_tabs)}`
        from async_fetch_and_parse / fetch_and_parse for the real yetracker.net
        Misc tab GID — this must be True so the gid-shortcut skips parse_sheet
        and falls through to full tab discovery instead."""
        tabs = _discover_named_tabs(YETRACKER_TAB_LISTING_JS)
        requested_gid = "70063278"  # what a URL like ".../#gid=70063278" carries
        gid_is_misc_tab = requested_gid in {g for g, _kind in _get_misc_tabs(tabs)}
